...
exports.run = async (bot, msg, args, rawArgs) => {
  ...
  if (args.length === 0) return msg.channel.send(`Error: Missing command arguments!, Type: \`${botConfig.prefix}help\` for more info`)
  ...
  if (msg.attachments.array()[0]) {
    ...
    return msg.channel.send(`Error: Unknown!, type: \`${botConfig.prefix}help\` for more info`);
  }
  ...
  if (sizeInMb > 45) return msg.channel.send(`Error: File size is larger than 45MB, i don't wanna suffer.`)
